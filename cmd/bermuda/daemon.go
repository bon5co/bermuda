...
const (
	...
	// defaultIndexEvery is how often the vault is re-hashed against the search
	// index. Minutes, not seconds: the notes are written by people and agents
	// finishing a piece of work, so a change that is five minutes old is still
	// the freshest thing anybody could ask about. The sweep itself is a
	// directory walk and a hash — no helper process starts unless a file
	// actually changed — which is what makes it cheap enough to leave on.
	defaultIndexEvery = 5 * time.Minute
)
...
type daemon struct {
	...
	// indexEvery is how often the vault is swept into the search index. Zero
	// means the default; a test sets it short.
	indexEvery time.Duration
	// indexVault brings the search index back in step. A field so a test can
	// drive the ticker without a Python on the machine.
	indexVault func(context.Context) (index.Report, error)
	// indexOff latches once the helper turns out to be missing. Saying it
...
//
// Nothing here is fatal. A machine with no Python cannot search its notes, and
// that is a smaller problem than a scheduler that stops running jobs because
// of it — so the first refusal turns the sweep off for this process and every
// other failure is reported and retried on the next tick.
func (d *daemon) sweepIndex(ctx context.Context) {
	if d.indexOff {
		return
	}
	fn := d.indexVault
	if fn == nil {
		...
		if _, err := os.Stat(c.Root()); err != nil {
			// No vault, nothing to index, and no index directory created for
			// an install that never asked for one.
			return
		}
		fn = func(ctx context.Context) (index.Report, error) { return c.Sweep(ctx, false, nil) }
	}
	rep, err := fn(ctx)
	if err != nil {
		if errors.Is(err, index.ErrNoPython) {
			d.indexOff = true
			fmt.Fprintln(os.Stderr, "bermuda: vault search is off —", helperHowTo)
			return
		}
		fmt.Fprintln(os.Stderr, "bermuda: index sweep:", err)
		return
	}
	if rep.Indexed > 0 || rep.Removed > 0 {
		fmt.Printf("bermuda: indexed %d note(s), removed %d, %d chunk(s) in %s\n",
			rep.Indexed, rep.Removed, rep.Chunks, rep.Took.Round(time.Millisecond))
	}
}
...
func (d *daemon) run(ctx context.Context) {
	...
	defer rt.Stop()
	indexEvery := d.indexEvery
	if indexEvery == 0 {
		indexEvery = defaultIndexEvery
	}
	it := time.NewTicker(indexEvery)
	defer it.Stop()
	for {
		select {
		case <-ctx.Done():
