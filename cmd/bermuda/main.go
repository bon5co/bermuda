...
Usage:
  ...
  bermuda memory path                 Where the notes live
  bermuda memory init [--vault <dir>] Create the memory directory and seed its index
  bermuda memory index                Index the vault for search; reindexes only what changed
  bermuda memory index --status       What is indexed, what is stale, and how Chroma is run
  bermuda memory search <question>    Find notes by meaning, not by wording
                     [--section memory] [--type project] [-n 8]

  Search is a Chroma collection kept in step with the vault: a note is chunked
  at paragraph level, keyed by its path, and reindexed when its SHA-256 stops
  matching. Chroma runs embedded -- a directory under ~/.bermuda/index, no
  server -- through a Python it finds itself, so a machine without one keeps
  every other command and simply has no search.
  ...
  flow's step state dies with the run. --blocked-on is the distinction it exists
  for -- "the agent has not done it" and "the agent cannot do it" look identical
  ...
  bermuda doctor                      Detached schedulers serving a store nothing can address
