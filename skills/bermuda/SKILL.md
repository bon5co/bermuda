...
bermuda memory search 'why was the browser tool replaced'   # by meaning, not wording
bermuda memory search 'pricing' --section memory --type project -n 5
bermuda memory index       # index what changed (the daemon also does this)
bermuda memory index --status
```

**Search before answering from your own head, and before re-deriving anything.**
The index covers the whole vault, not just the memory folder, so it reaches the
notes no session loads: a playbook written in March, a procedure someone else
recorded, the note that already decided the question being asked. Results come
back as `path · heading` plus the paragraph, so open the file only when the
paragraph is worth more context. It searches by meaning — the words in the
question do not have to be the words in the note, which is the whole reason to
use it over grep.
...
month — forum; assumed by every future session — memory; not finished yet — a
checklist, below. → `docs/memory.md`

### The overwatch — the agent that sees the whole run

**Every flow that runs agents has one, declared or not.** Each step is a fresh
agent that read only its own prompt; the overwatch is handed the flow as
declared, every step's outcome and note, and the artifacts of whatever went
wrong, and decides what the run does instead of parking.

```yaml
overwatch:
  model: opus          # the decision is usually harder than the steps
  watch: on_trouble    # default: only where the run would park. every_step = a
                       # reader on every result, one agent call per step
  budget: 3            # decisions per run; survives a resume
  timeout: 10m         # one consult
  allow: [retry, goto, park, abort]   # add skip to let it accept a failed step
  brief: |
    The deploy step is not safe to retry. Park instead.
```

It writes `decision.json` in `$BERMUDA_STEP_DIR`:
`{"decision":"goto","step":"write","why":"..."}`. Decisions are `retry`, `goto`,
`park`, `abort`, `continue`, and `skip` only when `allow` names it.

**If you are the overwatch:** park is the right answer whenever the cause is not
legible from what you can read. Read the step directories named in your brief
before deciding — a decision taken from the summary alone is a guess, and a
guess costs a whole run. You are not there to get the run finished: a run that
stops with a clear reason is a good outcome, a run that finishes on work nobody
checked is not.

**Writing a flow:** a declared `on_fail` edge still wins — it is explicit and
free, and the overwatch is asked only where the flow would park, including when
an edge runs out. Ambiguity parks: no file, bad JSON, a disallowed verb, a goto
that points forward. → `docs/flows.md`

## Checklists — what is still outstanding

**Any task that will produce more than two or three artifacts gets one, at the
start.** The failure this exists for is not losing work, it is being unable to
report it: a session that opened two PRs, filed two issues, moved a board card
and wrote three notes had all of it recorded somewhere and nothing enumerating
it, so the human asked four questions in a row to reconstruct state the harness
already held. Their words filing this: *"i have to keep asking you is this
done"*.
...
bermuda check tick 'open PR'          # or by number: check tick 1
bermuda check untick 2
bermuda check ls                      # every list with something open, one line each
bermuda check show                    # numbered items, for picking one
```
...
**`--blocked-on` is the point.** "I have not done it yet" and **"I cannot do
it"** look identical in every other record, and a list that cannot tell them
apart reads as an idle agent. Name a *person* and say what only they can do: a
PR needing a human approving review on a repo where you have `admin:false`, a
merge held for somebody's timing call. Add such an item blocked rather than
leaving it off — an item that is not on the page is one nobody knows is waiting.

Add items **as you learn them**, not at the end. A list written up afterwards is
a summary, and summaries are the thing that was already failing.

`<list>` is optional everywhere and the arity decides: `check tick 3` names an
item, `check tick ship-640 3` names both. Unnamed, it resolves `$BERMUDA_CHECK`
and then the most recent page — so one `check new` at the start of a task and no
later command needs an id. An ambiguous list or item is refused with the
candidates named, never guessed.

The page is a Markdown file in `checklists/` inside the memory vault, so a human
reads and ticks it in Obsidian with no CLI at all, and it survives your restart,
the flow run, and bermuda. A tick writes one byte, so running it while somebody
has the page open is safe. → `docs/checklists.md`

A flow step names its own item with `check: <text>`; `bermuda flow run <id>
--check <list>` seeds every such item before step one and ticks each as it
reports ok. Inside a step `$BERMUDA_CHECK` already points at the page, so
`bermuda check add '<what I just found>'` needs no id.
...
steps:
  ...
  - id: review
    check: review the diff        # ticked on the run's checklist when this step is ok
