...
type Run struct {
	...
	// CheckList is the path of the checklist page this run ticks as its steps
	// complete, empty when it is bound to none. Recorded on the run for the same
	// reason the thread is: a resume has to tick the same page the first attempt
	// did, and a resume is started by a command that was given no --check.
	CheckList string
}
