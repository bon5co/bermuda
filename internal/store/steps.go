...
type Step struct {
	...
	// Check names the checklist item this step is, ticked when the step reports
	// ok. It is legal on both kinds of step, because it configures the harness's
	// bookkeeping rather than an agent.
	//
	// The point is that the declared sequence and the human's view of it are one
	// object rather than two that drift. Without it a flow's progress lives only
	// in the run — which ends — and the checklist a person is reading has to be
	// ticked by somebody remembering to.
