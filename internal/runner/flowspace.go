...
// EnvCheck names the checklist page a step's shell acts on, so `bermuda check
// add` and `bermuda check tick` inside a step need no list id. Same bargain as
// EnvThread: a page id a step has to carry in its prompt is one it eventually
// gets wrong, and an item ticked on somebody else's page is worse than an item
// left open.
const EnvCheck = "BERMUDA_CHECK"
