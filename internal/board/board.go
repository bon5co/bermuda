...
type Model struct {
	...
	jobs []store.Job
	runs []store.Run
	// glance is the MEMORY tab's summary of the search index, read from the
	// index manifest rather than by scanning the vault. See index.Glance for
	// why the board must not do the scan itself.
	glance index.Glance
...
type Deps struct {
	...
	MemoryDir string
	// IndexDir is where the search index keeps its manifest, handed in for the
	// same reason as MemoryDir. Empty means this installation has no index and
	// the MEMORY tab simply says so.
	IndexDir string
...
type dataMsg struct {
	...
	memory memory.Stats
	// glance is the search index as the manifest reported it on this read. It
	// carries no error for the same reason memory does not: never indexed is a
	// state the tab renders, not a failure that should blank the board.
	glance index.Glance
	err    error
}
...
func (m *Model) load() tea.Cmd {
	...
	return func() tea.Msg {
		...
		// The index is read from its manifest, never by scanning the vault:
		// this goroutine may block, but it runs every three seconds, and
		// hashing a vault on that cadence would burn a percent of a core for
		// as long as the board is open to answer a question that has not
		// changed since the last daemon sweep.
		var glance index.Glance
		if m.deps.IndexDir != "" {
